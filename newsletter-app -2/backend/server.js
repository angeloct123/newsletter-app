...
if (IS_PRODUCTION && (!JWT_SECRET || !ADMIN_USERNAME || !ADMIN_PASSWORD)) {
  console.error('❌ ERRORE FATALE: JWT_SECRET, ADMIN_USERNAME e ADMIN_PASSWORD devono essere impostati in produzione!');
  process.exit(1);
}
...
// FIX: CSP pulita — rimossi unsafe-eval e CDN non necessari (no più iframe)
app.use(helmet({
...
async function initDB() {
  ...
  // FIX: Migrazione — aggiunge i nuovi campi se il DB esisteva già
  migrateDB();
  ...
  migrations.forEach(({ column, sql }) => {
    try {
      ...
      if (!columns.includes(column)) {
        db.run(sql);
        console.log(`✅ Migrazione: aggiunta colonna "${column}" a campaigns`);
      }
    } catch(e) {
      // La colonna potrebbe già esistere
    }
  });
}
...
function saveDB() {
  ...
  try {
    ...
    saveInProgress = false;
    console.error('❌ Errore export DB:', e.message);
  }
}
...
if (IS_PRODUCTION && (!smtpConfig.host || !smtpConfig.auth.user || !smtpConfig.auth.pass)) {
  console.error('⚠️ ATTENZIONE: Configurazione SMTP incompleta! Le email non funzioneranno.');
}
...
transporter.verify()
  .then(() => console.log('✅ Connessione SMTP verificata'))
  .catch(err => console.error('⚠️ SMTP non raggiungibile:', err.message));
...
app.get('/api/subscribers/count', authMiddleware, function(req, res) {
  try {
    const total = dbGet("SELECT COUNT(*) as count FROM subscribers", []);
    const active = dbGet("SELECT COUNT(*) as count FROM subscribers WHERE status = 'active'", []);
...
app.post('/api/subscribers', authMiddleware, function(req, res) {
  try {
    ...
    if (existing) return res.status(409).json({ error: 'Questa email è già registrata' });
...
// FIX: POST/PUT campagne — accetta sia html_content che htmlcontent per retrocompatibilità
...
app.put('/api/campaigns/:id', authMiddleware, function(req, res) {
  try {
    ...
    if (campaign.status === 'sent') {
      return res.status(400).json({ error: 'Non puoi modificare una campagna già inviata' });
    }
...
app.post('/api/campaigns/:id/send', authMiddleware, async function(req, res) {
  try {
    ...
    if (campaign.status === 'sent' || campaign.status === 'sending') {
      return res.status(400).json({ error: 'Campagna già inviata o in fase di invio' });
    }
...
async function processEmailQueue() {
  ...
  try {
    ...
    try {
      ...
      console.log(`✉️ Inviata a ${item.subscriber_email} (campagna: ${item.campaign_name})`);
    } catch(emailErr) {
      ...
      if (attempts >= 3) {
        ...
        console.error(`❌ Fallito definitivamente: ${item.subscriber_email} - ${emailErr.message}`);
      } else {
        dbRun("UPDATE email_queue SET status = 'pending', attempts = ?, last_error = ? WHERE id = ?",
          [attempts, emailErr.message, item.id]);
        console.warn(`⚠️ Tentativo ${attempts}/3 fallito per ${item.subscriber_email}: ${emailErr.message}`);
      }
    }
    ...
    if (remaining.count === 0) {
      ...
      console.log(`🎉 Campagna "${item.campaign_name}" completata! ${sentTotal.count} email inviate.`);
    }
  } catch(err) {
...
app.post('/api/test-smtp', authMiddleware, async function(req, res) {
  try {
    ...
    res.json({ message: 'Connessione SMTP riuscita! Il server è raggiungibile.' });
  } catch(err) {
...
app.get('/api/unsubscribe/:campaignId/:subscriberId', function(req, res) {
  try {
    ...
    <body><div class="card"><h1>✅ Iscrizione annullata</h1>
    <p>${subscriber ? '<strong>' + subscriber.email + '</strong><br>' : ''}Il tuo indirizzo è stato rimosso dalla nostra mailing list.</p>
    <p>Non riceverai più le nostre newsletter.</p></div></body></html>`);
  } catch(err) {
...
app.get('/api/stats', authMiddleware, function(req, res) {
  try {
    ...
    const totalSent = dbGet("SELECT COALESCE(SUM(total_sent), 0) as count FROM campaigns WHERE status = 'sent'", []) || { count: 0 };
    const sendingCampaigns = dbAll("SELECT * FROM campaigns WHERE status = 'sending'", []) || [];
    const recentCampaigns = dbAll("SELECT * FROM campaigns ORDER BY created_at DESC LIMIT 6", []) || [];

    // Ultimi 30 giorni — nuovi iscritti per giorno
    ...
    res.json({
      ...
      totalOpened: totalOpens.count,
      totalClicks: totalClicks.count,
      totalUnsubscribes: totalUnsubs.count,
      sendingCampaigns,
      recentCampaigns,
      subsGrowth
    });
...
async function start() {
  ...
  console.log('✅ Password admin hashata');

  await initDB();

  app.listen(PORT, function() {
    console.log(`🚀 Server avviato su porta ${PORT}`);
    console.log(`📊 Modalità: ${IS_PRODUCTION ? 'PRODUZIONE' : 'SVILUPPO'}`);
  });
}
